...
// Une date de fin strictement antérieure à aujourd'hui est considérée comme passée
function estDatePassee(dateStr) {
...
function Stagiaires() {
  ...
  // Empêche l'effet de réouverture (lié à location.state.editId) de se redéclencher
  // après une modification — voir useEffect plus bas.
  const editIdTraite = useRef(null);
  ...
  // Ouvre automatiquement le formulaire de modification si on arrive avec un
  // editId dans location.state (ex: depuis la fiche détail). On ne le fait
  // qu'UNE SEULE FOIS par editId (via editIdTraite), et on nettoie le state
  // avec navigate() — pas window.history.replaceState, qui ne met pas à jour
  // location.state côté React Router et provoquait la réouverture du dialogue
  // juste après chaque enregistrement (chargerStagiaires() déclenchait l'effet
  // à nouveau tant que location.state.editId restait présent).
  useEffect(() => {
    const editId = location.state?.editId;
    if (editId && stagiaires.length > 0 && editIdTraite.current !== editId) {
      const stagiaire = stagiaires.find((s) => s.id === editId);
      if (stagiaire) {
        ouvrirModification(stagiaire);
      }
      editIdTraite.current = editId;
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => {
      const nouveauForm = { ...prev, [name]: type === "checkbox" ? checked : value };
      // Si on modifie la date de fin et qu'elle tombe dans le passé,
      // on bascule automatiquement le statut sur "Terminé".
      if (name === "date_fin" && estDatePassee(value)) {
        nouveauForm.statut = "termine";
      }
      return nouveauForm;
    });
    if (errors[name]) {
  ...
  const handleSubmit = () => {
    ...
    const dataToSend = {
      ...
      // Filet de sécurité : même si le statut affiché n'a pas été touché,
      // une date de fin passée force "termine" à l'enregistrement.
      statut: estDatePassee(form.date_fin) ? "termine" : form.statut,
  ...
  const handleDelete = (id, e) => {
    ...
    fetch(`${API_URL}/stagiaires/${id}`, { method: "DELETE", headers: authHeaders() })
